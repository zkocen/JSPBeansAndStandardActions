...
public class CountryTest {
	...
	public void testMutatorsAndAccessors() {
		...
		assertEquals("Could not set continent as expected", continent,
				c.getContinent());
		// region = "Middle";
		c.setRegion(region);
		...
		assertEquals("Could not set surfaceArea as expected", surfaceArea,
				c.getSurfaceArea(), delta);
		// indepYear = 1776;
		c.setIndepYear(indepYear);
		assertEquals("Could not set indep year as expected", indepYear,
				c.getIndepYear());
		// population = 350000000;
		c.setPopulation(population);
		assertEquals("Could not set population as expected", population,
				c.getPopulation());
		// lifeExpectancy = 73;
		c.setLifeExpectancy(lifeExpectancy);
		assertEquals("Could not set life as expected", lifeExpectancy,
				c.getLifeExpectancy(), delta);
		// gnp = 15000000000000.0;
		c.setGnp(gnp);
		assertEquals("Could not set gnp as expected", gnp, c.getGnp(), delta);
		// gnpOld = 14000000000000.0;
		c.setGnpOld(gnpOld);
		assertEquals("Could not set gnp old as expected", gnpOld,
				c.getGnpOld(), delta);
		// localName = "USA";
		c.setLocalName(localName);
		assertEquals("Could not set local name as expected", localName,
				c.getLocalName());
		// governmentForm = "Democracy";
		c.setGovernmentForm(governmentForm);
		assertEquals("Could not set government as expected", governmentForm,
				c.getGovernmentForm());
		// headOfState = "The Current President";
		c.setHeadOfState(headOfState);
		assertEquals("Could not set head of state as expected", headOfState,
				c.getHeadOfState());
		// capital = "Washington, D. C.";
		c.setCapital(capital);
		assertEquals("Could not set capital as expected", capital,
				c.getCapital());
		// code2 = "123151515";
		c.setCode2(code2);
	...
	public void testToString() {
		...
		assertTrue(
				"To String does not contain expected value for surface area",
		...
		assertTrue(
				"To String does not contain expected value for lifeExpectancy",
		...
		assertTrue(
				"To String does not contain expected value for governmentForm",
